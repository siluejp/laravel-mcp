...
if not os.getenv("OPENAI_API_KEY"):
    print(
        ...
        "The AI assistant will not work."
    )

# Initialize the RAG service and LLM once when the module is loaded.
rag_service = RAGService()
try:
    llm = ChatOpenAI(model="gpt-4o")  # Using a powerful model
...
template = """
    ...
    Your primary goal is to help developers solve their problems. Analyze the
    user's question and the provided context to give the best possible answer.

    **If the user's query is an error message or a debug request:**
    ...
    **For all other questions (code generation, concepts, etc.):**
    1. Answer the user's question based *only* on the following context from the
       official Laravel 5.6 documentation.
    2. Provide concise, accurate, and helpful code examples where appropriate.
    3. If the answer cannot be found in the context, state that you do not have
       enough information from the provided documentation.
...
rag_chain = (
    {
        "context": lambda x: format_docs(
            rag_service.find_relevant_documents(x["question"])
        ),
...
def laravel_5_6_assistant(
    ...
    if rag_service.vector_store is None:
        return (
            "Error: The documentation vector store is not loaded. "
